# ═══════════════════════════════════════════════════════════════════════════════
# FINAL RESILIO-MAP WITH REAL FUTURE PROJECTIONS (FALLBACK TO SYNTHETIC)
# ═══════════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════════
# PAGE CONFIG & STYLING (same as before – kept intact)
# ═══════════════════════════════════════════════════════════════════════════════
...
# Current rasters directory (unchanged)
BIOCLIM_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'data', 'bioclim')
# Future rasters directories (new)
FUTURE_BASE = os.path.join(BIOCLIM_DIR, 'future')
FUTURE_SSP245 = os.path.join(FUTURE_BASE, 'ssp245')
FUTURE_SSP585 = os.path.join(FUTURE_BASE, 'ssp585')
...
# ═══════════════════════════════════════════════════════════════════════════════
# BIOCLIM FUNCTIONS (current + future, with fallback)
# ═══════════════════════════════════════════════════════════════════════════════
def check_current_rasters() -> bool:
    if not RASTERIO_AVAILABLE:
        return False
    return all(os.path.exists(os.path.join(BIOCLIM_DIR, f"{v}.tif")) for v in BIOCLIM_VARS)

def check_future_rasters(scenario: str) -> bool:
    """Return True if all 5 future rasters exist for a given scenario (ssp245 or ssp585)."""
    if not RASTERIO_AVAILABLE:
        return False
    future_dir = FUTURE_SSP245 if scenario == 'ssp245' else FUTURE_SSP585
    return all(os.path.exists(os.path.join(future_dir, f"{v}.tif")) for v in BIOCLIM_VARS)
...
def extract_bioclim_current(lat: float, lon: float, scientific_name: str = "") -> list[float]:
    """Extract current (1970-2000) bioclim values from data/bioclim/"""
    conn = init_db()
    cursor = conn.cursor()
    ...
    if cached:
        return list(cached)
    if not RASTERIO_AVAILABLE or not check_current_rasters():
    ...
    values = [round(v, 1) if isinstance(v, (int, float)) else v for v in values]
...
def extract_bioclim_future(lat: float, lon: float, scenario: str, year: str = "2050") -> list[float]:
    """
    Extract future bioclim values for a given scenario (ssp245 or ssp585).
    If future rasters are missing, falls back to synthetic scaling of current values.
    """
    if not RASTERIO_AVAILABLE or not check_future_rasters(scenario):
        # Fallback: synthetic scaling
        current = extract_bioclim_current(lat, lon)
        if scenario == 'ssp585':
            future = [current[0]*1.05, current[1]*0.95, current[2]*1.10, current[3]*0.85, current[4]*1.08]
        else:
            future = [current[0]*1.02, current[1]*0.98, current[2]*1.04, current[3]*0.92, current[4]*1.04]
        return [round(v, 1) for v in future]
    future_dir = FUTURE_SSP245 if scenario == 'ssp245' else FUTURE_SSP585
    values = []
    for var in BIOCLIM_VARS:
        try:
            filepath = os.path.join(future_dir, f"{var}.tif")
            with rasterio.open(filepath) as dataset:
                val = list(dataset.sample([(lon, lat)]))[0][0]
                if val is None or val == dataset.nodata or np.isnan(float(val)):
                    val = None
...
# ═══════════════════════════════════════════════════════════════════════════════
# MAXENT MODEL (presence-only using logistic regression with no penalty)
# ═══════════════════════════════════════════════════════════════════════════════
class MaxEntModel:
...
# ═══════════════════════════════════════════════════════════════════════════════
# IMPROVED PSEUDO-ABSENCE GENERATION (buffer around presence)
# ═══════════════════════════════════════════════════════════════════════════════
...
def get_background_points(n: int, presence_coords: list = None, buffer_km: float = 10.0) -> list[tuple]:
    if presence_coords is None:
        presence_coords = []
    bg_points = []
    max_attempts = n * 10
    attempts = 0
    rng = np.random.RandomState(42)
    lat_min, lat_max = 12.0, 19.5
    lon_min, lon_max = 119.0, 125.0
    while len(bg_points) < n and attempts < max_attempts:
        lat = rng.uniform(lat_min, lat_max)
        lon = rng.uniform(lon_min, lon_max)
        too_close = False
        for plat, plon in presence_coords:
            if haversine(lat, lon, plat, plon) < buffer_km:
                too_close = True
                break
        if not too_close:
            bg_points.append((lat, lon))
        attempts += 1
    if len(bg_points) < n:
        needed = n - len(bg_points)
        fallback = [(rng.uniform(lat_min, lat_max), rng.uniform(lon_min, lon_max)) for _ in range(needed)]
        bg_points.extend(fallback)
    return bg_points

# ═══════════════════════════════════════════════════════════════════════════════
# ML FUNCTIONS (train on current climate only)
# ═══════════════════════════════════════════════════════════════════════════════
def build_feature_matrix(species_key: str) -> tuple[np.ndarray, np.ndarray]:
    ...
    background_coords = get_background_points(n_presence, presence_coords, buffer_km=10.0)
    X_presence = np.array([extract_bioclim_current(lat, lon, species_key) for lat, lon in presence_coords])
    X_background = np.array([extract_bioclim_current(lat, lon, species_key) for lat, lon in background_coords])
...
# ═══════════════════════════════════════════════════════════════════════════════
# REAL REFUGIA COMPUTATION (using current + future rasters, fallback to synthetic)
# ═══════════════════════════════════════════════════════════════════════════════
def compute_real_refugia(species_key: str, scenario: str, n_points: int = 500) -> tuple:
    """
    Compute refugia metrics by predicting on current and future climate for random grid points.
    Returns (refugia_km2, gained_km2, maintained_km2, lost_km2, point_data)
    """
    if species_key not in st.session_state.model_results:
        X, y = build_feature_matrix(species_key)
        results = train_models(X, y)
        st.session_state.model_results[species_key] = results
    ...
    lat_min, lat_max = 12.0, 19.5
    lon_min, lon_max = 119.0, 125.0
    grid_points = [(rng.uniform(lat_min, lat_max), rng.uniform(lon_min, lon_max)) for _ in range(n_points)]
    current_suits = []
    future_suits = []
    for lat, lon in grid_points:
        curr_vals = extract_bioclim_current(lat, lon, species_key)
        curr_scaled = scaler.transform([curr_vals])
        p_maxent = maxent.predict_proba(curr_scaled)[0, 1]
        p_rf = rf.predict_proba(curr_scaled)[0, 1]
        p_xgb = xgb_model.predict_proba(curr_scaled)[0, 1]
        curr_suit = weights[0]*p_maxent + weights[1]*p_rf + weights[2]*p_xgb
        fut_vals = extract_bioclim_future(lat, lon, scenario, "2050")
        fut_scaled = scaler.transform([fut_vals])
        p_maxent_f = maxent.predict_proba(fut_scaled)[0, 1]
        p_rf_f = rf.predict_proba(fut_scaled)[0, 1]
        p_xgb_f = xgb_model.predict_proba(fut_scaled)[0, 1]
        fut_suit = weights[0]*p_maxent_f + weights[1]*p_rf_f + weights[2]*p_xgb_f
        current_suits.append(curr_suit)
        future_suits.append(fut_suit)
    cell_area_km2 = 1.0
    refugia_cnt = sum(1 for c, f in zip(current_suits, future_suits) if c >= 0.7 and f >= 0.7)
    gained_cnt = sum(1 for c, f in zip(current_suits, future_suits) if c < 0.5 and f >= 0.7)
    maintained_cnt = sum(1 for c, f in zip(current_suits, future_suits) if c >= 0.5 and f >= 0.5 and not (c >= 0.7 and f >= 0.7))
    lost_cnt = sum(1 for c, f in zip(current_suits, future_suits) if c >= 0.5 and f < 0.5)
    refugia_km2 = refugia_cnt * cell_area_km2
    gained_km2 = gained_cnt * cell_area_km2
    maintained_km2 = maintained_cnt * cell_area_km2
    lost_km2 = lost_cnt * cell_area_km2
    point_data = [(gp[0], gp[1], cs, fs) for gp, cs, fs in zip(grid_points, current_suits, future_suits)]
    return refugia_km2, gained_km2, maintained_km2, lost_km2, point_data

def build_refugia_map(species_key: str, scenario: str, point_data: list) -> folium.Map:
    """Build map using real prediction points."""
    ...
    for lat, lon, cur_suit, fut_suit in point_data:
        if cur_suit >= 0.7 and fut_suit >= 0.7:
            cat = 'refugium'
        elif cur_suit < 0.5 and fut_suit >= 0.7:
            cat = 'gained'
        elif cur_suit >= 0.5 and fut_suit >= 0.5:
            cat = 'maintained'
        else:
            cat = 'lost'
        folium.CircleMarker(location=[lat, lon], radius=4, color=dot_colors[cat], fill=True, fill_color=dot_colors[cat],
                            fill_opacity=0.5, weight=1, tooltip=f"{cat.capitalize()} · cur={cur_suit:.2f} fut={fut_suit:.2f}").add_to(m)
    return m

# ═══════════════════════════════════════════════════════════════════════════════
# STABILITY CHART, RECOMMENDATIONS, EXPORTS (same as before)
# ═══════════════════════════════════════════════════════════════════════════════
...
def get_recommendations(refugia: int, gained: int, maintained: int, lost: int, species_name: str, nipas_pct: float = 0.60, sc_key: str = 'ssp245') -> list[dict]:
    ...
    if lost_pct >= 20:
        ...
        recommendations.append({"priority": priority, "title": "Mitigate Habitat Loss in Climate-Vulnerable Zones",
                                "description": f"Approximately {lost:,.0f} km² of currently suitable habitat is projected to become climatically unsuitable for {species_name} by 2050. DENR-BMB should implement habitat corridor programs.",
                                "tags": ["LGU", "Policy"]})
    if gained >= 50:
        recommendations.append({"priority": "MEDIUM", "title": "Protect Emerging Climate-Suitable Zones",
                                "description": f"The system identifies {gained:,.0f} km² of newly suitable habitat (shown in blue) that is currently unoccupied but projected to become climatically viable for {species_name} by 2050.",
                                "tags": ["LGU", "NIPAS"]})
    if sc_key == 'ssp585':
...
def generate_excel_export(species_name, common_name, sp_class, records, ens_auc,
    ...
    if not OPENPYXL_AVAILABLE:
        ...
        return None
    df_245 = pd.DataFrame([{'Scenario': 'SSP2-4.5', 'Refugia_km2': ref_245, 'Gained_km2': gain_245, 'Maintained_km2': main_245, 'Lost_km2': lost_245, 'AUC_ROC': ens_auc, 'NIPAS_Within_Pct': nipas_pct, 'NIPAS_Outside_Pct': 1-nipas_pct}])
    df_585 = pd.DataFrame([{'Scenario': 'SSP5-8.5', 'Refugia_km2': ref_585, 'Gained_km2': gain_585, 'Maintained_km2': main_585, 'Lost_km2': lost_585, 'AUC_ROC': ens_auc, 'NIPAS_Within_Pct': nipas_pct, 'NIPAS_Outside_Pct': 1-nipas_pct}])
...
conn = init_db()
current_ok = check_current_rasters()
if not current_ok and RASTERIO_AVAILABLE:
    st.warning("Current climate rasters not found in data/bioclim/. Using synthetic climate data for training. Please add current .tif files for reliable predictions.")
...
# ═══════════════════════════════════════════════════════════════════════════════
# HABITAT ANALYSIS PAGE (unchanged)
# ═══════════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════════
# RISK ASSESSMENT PAGE (real refugia with fallback)
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    with col1:
        sp_dash_raw = st.selectbox("Select Species", all_species, key="dash_sp")
    with col2:
    ...
    if st.session_state.dash_generated and st.session_state.dash_sp_key:
        ...
        # Compute real refugia (fallback to synthetic if future rasters missing)
        try:
            refugia_245, gained_245, maintained_245, lost_245, grid_245 = compute_real_refugia(sp_key, 'ssp245', n_points=600)
            refugia_585, gained_585, maintained_585, lost_585, grid_585 = compute_real_refugia(sp_key, 'ssp585', n_points=600)
            synthetic_fallback = False
        except Exception as e:
            # Fallback to synthetic numbers (old method) if anything fails
            synthetic_fallback = True
            st.warning(f"Future rasters not found or error. Using synthetic stability estimates. Error: {str(e)}")
            def stability_synthetic(sp_key, sc_key):
                np.random.seed(hash(sp_key + sc_key) % (2**31))
                base = len(get_occurrences(sp_key)) * 22
                shift = 1.0 if sc_key == 'ssp245' else 0.62
                ref = int(base * shift * np.random.uniform(0.92, 1.08))
                gain = int(base * 0.15 * np.random.uniform(0.8, 1.2))
                main = int(base * 0.30 * np.random.uniform(0.9, 1.1))
                lost = int(base * (1 - shift) * np.random.uniform(0.9, 1.1) + 180)
                return ref, gain, main, lost
            refugia_245, gained_245, maintained_245, lost_245 = stability_synthetic(sp_key, 'ssp245')
            refugia_585, gained_585, maintained_585, lost_585 = stability_synthetic(sp_key, 'ssp585')
            # Create dummy point data for map (use old distance-based circles to avoid breaking map)
            from random import uniform
            grid_245 = [(uniform(12,19.5), uniform(119,125), 0.6, 0.6) for _ in range(200)]
            grid_585 = [(uniform(12,19.5), uniform(119,125), 0.6, 0.6) for _ in range(200)]

        # Generate recommendations and NIPAS overlap (random for demo)
        ...
        with left_col:
            ...
            m245 = build_refugia_map(sp_key, 'ssp245', grid_245)
            st_folium(m245, height=380, returned_objects=[])
            st.markdown("**Habitat Metrics**")
            m1,m2,m3,m4 = st.columns(4)
            with m1: st.markdown(f"""<div style="background:#eaf4ed;border-radius:8px;padding:12px;text-align:center;"><div class="fraunces-number" style="font-size:24px;color:#1e6b3c;">{refugia_245:,.0f}</div><div style="font-size:10px;">Refugia (km²)</div></div>""", unsafe_allow_html=True)
            with m2: st.markdown(f"""<div style="background:#eff6ff;border-radius:8px;padding:12px;text-align:center;"><div class="fraunces-number" style="font-size:24px;color:#2563eb;">{gained_245:,.0f}</div><div style="font-size:10px;">Gained (km²)</div></div>""", unsafe_allow_html=True)
            with m3: st.markdown(f"""<div style="background:#f7f9f7;border-radius:8px;padding:12px;text-align:center;"><div class="fraunces-number" style="font-size:24px;color:#8fa893;">{maintained_245:,.0f}</div><div style="font-size:10px;">Maintained (km²)</div></div>""", unsafe_allow_html=True)
            with m4: st.markdown(f"""<div style="background:#fef2f2;border-radius:8px;padding:12px;text-align:center;"><div class="fraunces-number" style="font-size:24px;color:#dc2626;">{lost_245:,.0f}</div><div style="font-size:10px;">Lost (km²)</div></div>""", unsafe_allow_html=True)
        ...
        with right_col:
            ...
            m585 = build_refugia_map(sp_key, 'ssp585', grid_585)
            st_folium(m585, height=380, returned_objects=[])
            st.markdown("**Habitat Metrics**")
            m1,m2,m3,m4 = st.columns(4)
            with m1: st.markdown(f"""<div style="background:#eaf4ed;border-radius:8px;padding:12px;text-align:center;"><div class="fraunces-number" style="font-size:24px;color:#1e6b3c;">{refugia_585:,.0f}</div><div style="font-size:10px;">Refugia (km²)</div></div>""", unsafe_allow_html=True)
            with m2: st.markdown(f"""<div style="background:#eff6ff;border-radius:8px;padding:12px;text-align:center;"><div class="fraunces-number" style="font-size:24px;color:#2563eb;">{gained_585:,.0f}</div><div style="font-size:10px;">Gained (km²)</div></div>""", unsafe_allow_html=True)
            with m3: st.markdown(f"""<div style="background:#f7f9f7;border-radius:8px;padding:12px;text-align:center;"><div class="fraunces-number" style="font-size:24px;color:#8fa893;">{maintained_585:,.0f}</div><div style="font-size:10px;">Maintained (km²)</div></div>""", unsafe_allow_html=True)
            with m4: st.markdown(f"""<div style="background:#fef2f2;border-radius:8px;padding:12px;text-align:center;"><div class="fraunces-number" style="font-size:24px;color:#dc2626;">{lost_585:,.0f}</div><div style="font-size:10px;">Lost (km²)</div></div>""", unsafe_allow_html=True)
