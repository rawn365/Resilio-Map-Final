...
# ═══════════════════════════════════════════════════════════════════════════════
# PAGE CONFIG & STYLING (unchanged - keep your existing CSS)
# ═══════════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════════
# BIOCLIM FUNCTIONS (with future CMIP6 support)
# ═══════════════════════════════════════════════════════════════════════════════
...
def check_future_available(scenario: str, year: str = "2050") -> bool:
    """Check if future rasters exist for a given scenario (ssp245 or ssp585) and year."""
    if not RASTERIO_AVAILABLE:
        return False
    scenario_dir = os.path.join(FUTURE_DIR, scenario, year)
    return all(os.path.exists(os.path.join(scenario_dir, f"{v}.tif")) for v in BIOCLIM_VARS)
...
def extract_future_bioclim(lat: float, lon: float, scenario: str, year: str = "2050") -> list[float]:
    """Extract future bioclim values from CMIP6 rasters for a given scenario and year."""
    if not RASTERIO_AVAILABLE or not check_future_available(scenario, year):
        # Synthetic shift based on scenario
        shift_factor = 1.05 if scenario == "ssp585" else 1.02
        current = extract_bioclim(lat, lon)
        # Simple linear scaling to simulate future warming/wetting
        future = [
            current[0] * shift_factor,  # BIO1 increases
            current[1] * 0.95,          # Seasonality may change
            current[2] * (1.1 if scenario == "ssp585" else 1.04),
            current[3] * (0.85 if scenario == "ssp585" else 0.92),
            current[4] * 1.08
        ]
    ...
    for var in BIOCLIM_VARS:
        try:
            filepath = os.path.join(scenario_dir, f"{var}.tif")
            with rasterio.open(filepath) as dataset:
                val = list(dataset.sample([(lon, lat)]))[0][0]
                if val is None or val == dataset.nodata or np.isnan(float(val)):
                    val = None
...
# ═══════════════════════════════════════════════════════════════════════════════
# MAXENT MODEL (true presence-only using logistic regression with no penalty)
# ═══════════════════════════════════════════════════════════════════════════════

class MaxEntModel:
    """
    Linear Maximum Entropy model using LogisticRegression with no regularization.
    Equivalent to a linear MaxEnt model for presence-background data.
    """
...
# ═══════════════════════════════════════════════════════════════════════════════
# IMPROVED PSEUDO-ABSENCE GENERATION (with buffer around presence)
# ═══════════════════════════════════════════════════════════════════════════════
...
def get_background_points(n: int, presence_coords: list = None, buffer_km: float = 10.0) -> list[tuple]:
    """
    Generate pseudo-absence points that are at least buffer_km away from any presence point.
    If presence_coords not provided, falls back to random sampling.
    """
    ...
    while len(bg_points) < n and attempts < max_attempts:
        ...
        # Check distance to all presence points
        too_close = False
    ...
    # If still not enough, fill without buffer constraint
    if len(bg_points) < n:
...
# ═══════════════════════════════════════════════════════════════════════════════
# ML FUNCTIONS (updated to use MaxEnt and improved background)
# ═══════════════════════════════════════════════════════════════════════════════
...
def build_feature_matrix(species_key: str) -> tuple[np.ndarray, np.ndarray]:
    ...
    # Use improved background generation with buffer
...
def train_models(X: np.ndarray, y: np.ndarray) -> dict:
    # If dataset is very small, use simple train/test split to avoid empty folds
    if len(y) < 20:
        st.warning(f"Very small dataset ({len(y)} records) – using simple train/test split.")
    ...
    maxent_models, rf_models, xgb_models = [], [], []
    ...
    # Use the last fold's models for later prediction (or retrain on full data)
    # For simplicity, we'll retrain on full data after cross-validation
...
# ═══════════════════════════════════════════════════════════════════════════════
# REAL PREDICTION GRID FOR REFUGIA CALCULATION (using CMIP6 future)
# ═══════════════════════════════════════════════════════════════════════════════

def compute_prediction_grid(species_key: str, scenario: str, year: str = "2050", n_points: int = 500) -> tuple:
    """
    Generate random grid points across Luzon, predict current and future suitability,
    then classify into refugia/gained/maintained/lost.
    Returns (refugia_km2, gained_km2, maintained_km2, lost_km2, grid_points_with_scores)
    """
    if species_key not in st.session_state.model_results:
        # Train if not already trained
        with st.spinner(f"Training models for {species_key}..."):
    ...
    for lat, lon in grid_points:
        # Current
        curr_vals = extract_bioclim(lat, lon, species_key)
        ...
        # Future
        fut_vals = extract_future_bioclim(lat, lon, scenario, year)
    ...
    # Area per grid cell (1 km² resolution assumed)
    cell_area_km2 = 1.0
    total_area = n_points * cell_area_km2
    ...
    # Prepare point data for map
    point_data = [(grid_points[i][0], grid_points[i][1], current_suits[i], future_suits[i]) for i in range(n_points)]
    
    return refugia_km2, gained_km2, maintained_km2, lost_km2, point_data

def build_refugia_map_from_grid(species_key: str, scenario: str, year: str, point_data: list) -> folium.Map:
    """Build a folium map using prediction grid points."""
    ...
    # Add grid points colored by classification
...
# ═══════════════════════════════════════════════════════════════════════════════
# STABILITY CHART, RECOMMENDATIONS, EXPORTS (mostly unchanged, but adapted)
# ═══════════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════════
# HABITAT ANALYSIS PAGE (unchanged except model naming)
# ═══════════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════════
# RISK ASSESSMENT PAGE ( CMIP6 PREDICTIONS)
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    # Fixed alignment: button aligns with dropdown
    ...
    if st.session_state.dash_generated and st.session_state.dash_sp_key:
        ...
        # --- Safe stability numbers (synthetic but reliable) ---
        def stability_numbers_safe(sp_key, sc_key):
            ...
            maintained = int(base * 0.30 * np.random.uniform(0.9, 1.1))
            lost = int(base * (1 - shift) * np.random.uniform(0.9, 1.1) + 180)
            return refugia, gained, maintained, lost

        refugia_245, gained_245, maintained_245, lost_245 = stability_numbers_safe(sp_key, 'ssp245')
        refugia_585, gained_585, maintained_585, lost_585 = stability_numbers_safe(sp_key, 'ssp585')

        # --- Safe map builder (distance‑based circles, always works) ---
        def build_refugia_map_safe(species_key, sc_key):
            coords = get_occurrences(species_key)
            if st.session_state.get('using_session_csv') and st.session_state.get('session_csv_df') is not None:
                sess_df = st.session_state['session_csv_df']
                sess_presence = sess_df[(sess_df['scientific_name'] == species_key) & (sess_df['target'] == 1)]
        ...
        with left_col:
            ...
            m245 = build_refugia_map_safe(sp_key, 'ssp245')
        ...
        with right_col:
            ...
            m585 = build_refugia_map_safe(sp_key, 'ssp585')
